...
public class BasicPlayerWeaponFactory extends WeaponFactory {
    ...
    public void adjustProjectileTemplate() {
        ...
        projectileTemplate.setCollider(new Collider(new Circle(new Vector2(projectileTemplate.getSprite().getPosition()).add(new Vector2(projectileTemplate.getSprite().getWidth(), projectileTemplate.getSprite().getHeight()).scl(0.5f)), projectileTemplate.getSprite().getHeight() / 2)));
    }
